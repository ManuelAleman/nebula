...
import { Cloud, Copy, Download, File, Info } from "lucide-react";
...
const PublicFileView: React.FC = () => {
    const [file, setFile] = useState<FileMetadataDTO>();
    ...
    useEffect(() => {
        const getFile = async () => {
            ...
            try {
                const res = await apiPrivate.get<FileMetadataDTO>(apiEndpoints.PUBLIC_FILE_VIEW(fileId));
                console.log(res)
                setFile(res.data);
                setError(null);
            } catch (err) {
    ...
    const handleDownload = async () => {
        ...
        try {
            const response = await apiPrivate.get(apiEndpoints.DOWNLOAD_FILE(fileId), {
            responseType: "blob", 
        });
            ...
            <header className="p-4 border-b bg-white">
                <div className="container mx-auto flex justify-between items-center">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="font-bold text-xl text-gray-800">Nebula</span>
                    </div>
                    <button
